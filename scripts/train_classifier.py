...
def train_model(model, dataloaders, sizes, criterion, optimizer, scheduler, num_epochs=25):
    since = time.time()
...
def main():
    ...
    datasets = {
        'train': CustomDataset(df_train, IMAGE_DIR, transform_train),
        'val': CustomDataset(df_val, IMAGE_DIR, transform_val),
    }
    ...
    phases = ['train', 'val']
    dataloaders = {p: DataLoader(datasets[p], **loader_settings) for p in phases}
    sizes = {p: len(datasets[p]) for p in phases}
